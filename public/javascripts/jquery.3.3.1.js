...
function winnow (elements, qualifier, not) {
	if (isFunction(qualifier)) {
		return jQuery.grep(elements, (elem, i) => Boolean(qualifier.call( elem, i, elem )) !== not);
	}
...
function adoptValue (value, resolve, reject, noValue) {
	...
	try {
		...
		if (value && isFunction(method = value.promise)) {
			method.call(value).done(resolve).
fail(reject);
...
function cloneCopyEvent (src, dest) {
	let events, i, l, pdataCur, pdataOld, type, udataCur, udataOld;
...
function propFilter (props, specialEasing) {
	let easing, hooks, index, name, value;
...
jQuery.fn.extend({
	"addClass" (value) {
		let classes, clazz, cur, curValue, elem, finalValue, 
i = 0,
			j;
...
i = 0,
	...
	"toggleClass" (value, stateVal) {
		...
		return this.each(function () {
			let className, classNames, i, self;
...
"scrollTop": "pageYOffset"}, (method, prop) => {
